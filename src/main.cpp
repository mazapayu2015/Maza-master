...
// Copyright (c) 2013 Maza developers
...
/** Amount of Mazas spent by the transaction.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MazaMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running MazaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
